...
def run_morphology_ensemble():
    print("Phase 13: Morphological Scaling USDM Ensemble")
    df = pd.read_csv("data/standardized_cosmos_v2.csv").dropna(subset=['r_kpc', 'v_obs', 'v_disk', 'sb_disk'])
    galaxies = df['galaxy'].unique()
    
    results = []
    
    # Physics: Morphologically-Aware Saturation
    def g_usdm_morph(r, cs, sb_d):
        # Saturation radius scales with stellar surface brightness
        r_sat = 2.5 * (sb_d / 100.0)**0.25
        coupling = 1.0 + np.log1p(r_sat / (r + 0.1))
        return (cs**2 / r) * coupling * 1.15e-10
    ...
    def obj_func(params, r, v_obs, v_gas, v_disk, v_bul, sb_d):
        ...
        g_pred = g_bar + g_usdm_morph(r, 80.0, sb_d.mean())
    ...
    final_df.to_csv("output/phase13_final_ml_fits.csv", index=False)

if __name__ == '__main__':
    run_morphology_ensemble()
